...
function getLowestFScoreNode(list: ICell[]) {
	return list.sort((nodeA, nodeB) => nodeA.fScore - nodeB.fScore)[0];
}
...
export function AstarSearch(
	...
	while (openList.length > 0) {
		let lowestFscoreNode = getLowestFScoreNode(openList);
		if (equalNodes(lowestFscoreNode, finishNode)) {
			return closedList;
		}
		openList = openList.filter((node) => !equalNodes(node, lowestFscoreNode));
		let nodeNeighbors = getNodeNeighbors(lowestFscoreNode, grid);
		for (let neighbor of nodeNeighbors) {
			let tentativeGScore = euclideanDistance(neighbor, lowestFscoreNode);
			if (tentativeGScore < neighbor.gScore) {
				closedList.push(neighbor);
				neighbor.previousNode = lowestFscoreNode;
